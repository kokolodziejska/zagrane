...
const extractRowData = (data: Row): RowValues[] => {
    ...
    return data.row_datas.map(r => {
        const rowData: RowValues = [
            ...
            getVal(r, 'notes'),
            getVal(r, 'additionals'), // UWAGA: pole na dodatkowe kolumny TO DO
        ];

        return rowData.map(item => (item === null || item === undefined ? '' : String(item)));
    });
};
...
async function get_table_data(tableId: number): Promise<RowValues[]> {
    try {
        ...
        const processedRows: RowValues[] = data.department_tables.flatMap(departmentTable => 
            departmentTable.rows.flatMap(row => 
                extractRowData(row)
            )
        );
...
function MangeBudget() {
    ...
    // 1. DODANIE FUNKCJI handleCellChange
    const handleCellChange = (rowIndex: number, colIndex: number, newValue: string) => {
        setTableRows(prevRows => {
            const newRows = [...prevRows];
            const newRow = [...newRows[rowIndex]];
            
            // Konwersja do stringu jest bezpieczna, ponieważ RowValues jest tablicą string | number
        ...
        // TUTAJ W PRZYSZŁOŚCI DODAJ LOGIKĘ WYSYŁANIA ZMIAN NA BACKEND
    };
    ...
    return (
        <div>
            ...
            <div className="flex flex-col justify-end items-end w-[80vw]">
                <Button variant="default" className="h-[3.5vh] w-[7vw]" onClick={() => { /* TODO: Implement Excel export */ }}>
                    Wygneruj Excel
                </Button>
            </div>
                    ...
                    <TableHeader className="bg-gray-100 sticky top-0 z-10">
                        <TableRow> 
                            {headers.map((header, j) => (
                                <TableHead
                                    key={j}
                                    className="px-2 py-2 text-left font-bold text-gray-800 border-x border-y max-w-60 whitespace-normal"
                                >
                                    {header}
                                </TableHead>
                            ))}
                    ...
                    <TableBody>
                        {tableRows.map((row, r) => (
                            <TableRow key={r} className="hover:bg-gray-50">
                                {row.map((ele, e) => (
                                    // 2. NAPRAWIONE: Użycie TableCell jako kontenera
                                    <TableCell
                                        key={e}
                                        className={`px-2 py-1 text-left border-x border-y max-w-60 ${
                                            e === 0 ? 'pl-4' : ''
                                        }`}
                                    >
                                        <input
                                            type="text"
                                            // NAPRAWIONE: Konwersja ele na string, aby uniknąć błędu TS/React o typach null/number
                                            value={String(ele)} 
                                            onChange={(event) => 
                                                handleCellChange(r, e, event.target.value)
                                            }
                                            // Stylizacja inputa, aby wyglądał jak zwykły tekst
                                            className="w-full h-full p-0 m-0 border-none bg-transparent focus:ring-0 focus:outline-none truncate"
                                        />
                                    </TableCell>
                                ))}
                            </TableRow>
